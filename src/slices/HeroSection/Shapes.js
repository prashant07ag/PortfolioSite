...
export function Shapes() {
  return (
    <div className="row-span-1 justify-center row-start-1 aspect-square  md:col-span-1 md:col-start-2 md:mt-5 ">
      <Canvas
        className="z-0"
        shadows
        gl={{ antialias: false }}
...
function Geometries() {
  const dogModel = useGLTF('/dog.glb'); // Load the dog model
...
function Geometry({ r, position, geometry, soundEffects, materials, dogModel }) {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      gsap.to(meshRef.current.rotation, {
        y: Math.PI * 8, // Rotate 10 full circles around the y-axis (5 times)
        duration: 2, // Duration of rotation animation
